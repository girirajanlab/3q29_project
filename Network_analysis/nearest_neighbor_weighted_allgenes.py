...
#For example, to find the connectivity of NCBP2 (Entrez ID 22916) to all other genes in the genome, one would run:
...
for gene_name in gene_targets:
	...
	outwriter = csv.writer(file(gene_name+"_nearest_neighbor_weighted.txt",'w'),delimiter='\t')
